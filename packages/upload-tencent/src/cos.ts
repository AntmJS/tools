...
if (fs.existsSync(configPath)) {
  ...
  if (antmConfig.uploadTencent && typeof antmConfig.uploadTencent === 'object') {
...
function generateTargetFilePath(targetDir = '', baseDir = '', filePath = '', replace) {
...
function generateTargetFilePaths(targetDir, dir, excludeRegexp) {
  ...
  return files
    ...
    .map(item => {
      return generateTargetFilePath(targetDir, dir, npath.join(dir, item), false)
    })
}
      ...
      cos.uploadFiles(
        ...
        function (err) {
          if (err) {
            ...
            console.info(
              chalk.green(
                `Request path: https://${npath.join(bucketConfig.CDN, targetDir, '...')}`,
              ),
            )
            resolve('')
          }
      ...
      cos.sliceUploadFile(
        ...
        function (err) {
          if (err) {
            ...
            console.info(chalk.green('Upload success'))
            console.info(
              chalk.green(`Request path: https://${npath.join(bucketConfig.CDN, target.Key)}`),
            )
            resolve('')
          }
        },
...
async function upload(program) {
  ...
  if (confirm) {
    try {
      // start 微信上传
      if (program.dir) {
        const files = generateTargetFilePaths(program.targetDir, program.dir, program.excludeRegexp)
